...
pub fn create_router(state: GatewayState) -> Router {
    Router::new()
        ...
        .route("/api/v1/budgets", any(proxy_budget))
        .route("/api/v1/budgets/*path", any(proxy_budget))
        .with_state(state)
}
...
async fn proxy_budget(
    ...
    // Rewrite path: /api/v1/budgets -> /budgets on downstream
    let downstream_path = path.strip_prefix("/api/v1").unwrap_or(path);
    let target_url = format!("{}{}{}", state.budget_url, downstream_path, query);
