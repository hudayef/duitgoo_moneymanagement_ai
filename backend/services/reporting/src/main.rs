...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    init_logging();
    init_tracing("reporting_service");

    let config = AppConfig::from_env().unwrap_or(AppConfig {
         database_url: std::env::var("DATABASE_URL").unwrap_or_else(|_| "postgres://duitgoo_user:duitgoo_password@localhost:5432/reporting_db".to_string()),
         server_port: 8009,
